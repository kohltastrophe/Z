...
- **Flexible:** Supports dictionaries, arrays (`Z { Z.f64 }`), maps (`Z { [Z.uint] = Z.u16 }`), migrating schemas, nested schemas, optionals (`Z.some`), and safe deserialization.
...
local PlayerDataSchema = Z {
	...
	Inventory = Z(8) { [Z.u8] = Z.u8 }, -- Schema map of u8 item ids to a u8 stack count, with a limit of 2^8 - 1 key/value pairs using Z(8)
}
...
| `Z(8/16/32){[ZType]=ZType}` | Map ZType (max items 2^8/16/32 - 1)        | 1/2/4 + N (ZTypeK + ZTypeV) |
| **Roblox Data Types**       |                                            |                             |
